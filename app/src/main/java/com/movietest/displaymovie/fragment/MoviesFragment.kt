...
import com.movietest.displaymovie.databinding.FragmentMoviesBinding
...
class MoviesFragment : Fragment(), PopupMenu.OnMenuItemClickListener {
    ...
    private fun setAdapter() {
        ...
        binding!!.mainRecycler.addOnScrollListener(object : PaginationScrollListener(linearLayoutManager!!) {
            override fun loadMoreItems() {
                this@MoviesFragment.isLoading = true
                currentPage += 1
            ...
            override val isLastPage: Boolean
                get() = this@MoviesFragment.isLastPage
            override val isLoading: Boolean
                get() = this@MoviesFragment.isLoading
        })
    }
    ...
    private fun loadLanguage(language: String) {
        save(context, "language", language)
        loadFirstPage()
    }

    private fun loadFirstPage() {
        val language = read(context, "language", "en")
        val s = read(context, "search", "no")
        if (s != "no") {
            callFromSearch(sortingType, s, language)!!.enqueue(object : Callback<AllMovies?> {
            ...
            callTopRatedMoviesApi(sortingType, language)!!.enqueue(object : Callback<AllMovies?> {
    ...
    private fun callTopRatedMoviesApi(sortingType: String?, language: String?): Call<AllMovies?>? {
